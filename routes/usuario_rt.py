...
from dto.usuario import UsuarioDto, UsuarioUpdateDTO

router = APIRouter()

@router.post("/usuario")
def crear_usuario(usuario: UsuarioDto, db: Session = Depends(get_db)):
    nuevo_usuario = Usuario(nombre=usuario.nombre, edad=usuario.edad, role=usuario.role)
